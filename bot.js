// hopefully this works @tbone

const Discord = require('discord.js');
const { Client, Collection } = require('discord.js');
const bot = new Discord.Client();

const token = 'ODAxMjA4ODU5Mzc3MDA4NjYx.YAdV5Q.lu8mlVKe4I0pNm2TRU17prR6RD8';
const prefix = '=';
...
bot.on('ready', () =>{
  console.log('Chill Bot Online!');
  bot.user.setActivity('Chill Music', {type: 'PLAYING'}).catch(console.error);
});
...
bot.on("message", message=>{
  if (!message.content.startsWith(prefix)) return;
    ...
    if (command === 'credits') {
        message.channel.send('This bot was written by pr#1951 and ﬂop#0001 in discord.js')
    }
    if(command === 'avatar') {
    ...
    if(command === 'purge') {
      ...
      if (!args.length) {
            return message.channel.send(`Please select an amount of messages to purge ${message.author}`);
        }
  
      if (!message.member.hasPermission("MANAGE_MESSAGES"))
        return message.reply("You don't have the required permission(s): Manage Messages");
     ...
     if(command === 'withrole') {
      ...
      if (!roleName) return message.channel.send(':x: Please specify a role');
        let membersWithRole = message.guild.members.filter(member => { 
            return member.roles.find("name", roleName).catch(message.channel.send(':x: Could not find role.'))
        }).map(member => {
     ...
     if(command === 'userinfo') {
      const mﬂop
#0001ember = message.guild.member(user);
      const embed = new Discord.RichEmbed()
          .setColor("RANDOM")
    ...
    if (command === 'youtube') {
      const embed = new Discord.RichEmbed()
        .setColor("RANDOM")
        .addField("Our YouTube channel: [Link](https://www.youtube.com/channel/UC8uaAVMmlV9V5kjD2yHk41A)")
        .setFooter(`Replying to ${message.author.username}#${message.author.discriminator}`)
       return message.channel.send({embed});
  }
});
