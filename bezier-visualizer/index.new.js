...
var t = 0,
  ...
  settings = {
      speed: 0.002,
      ease: window.quadraticEaseInOut
  }
...
function line(startx, starty, finishx, finishy) {
  ...
  canvas.context.moveTo(startx, starty, 0);
...
function removePointHandler(evt) {
  ...
  for (var i = 0; i < points.data.length; i++) {
   // console.log(x+' '+y+'\n'+points.data[i][0]+' '+points.data[i][1]+'\n'+intersectingPoints(x, y, i))
      ...
      console.log(intersectingPoints(x, y, i) === true )
      if (
        ...
        console.log(dragging)
              ...
              dragging = -1
              break;
      } else if (intersectingPoints(x,y,i) === false && dragging !== -1) {
        addPoint(x,y)
