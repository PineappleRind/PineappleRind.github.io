...
/*for (var i = 0; i < points.data.length; i++) {
  for (var j = 0; j < points.data.length; j++) points.computed.push([0]);
}
var save = {
  get: function () {
...
function advance() {
  ...
  // }
  /* var topMiddlePointX = lerp(points.data[0][0], points.data[2][0], easedT);
  var topMiddlePointY = lerp(points.data[0][1], points.data[2][1], easedT);
  point(topMiddlePointX, topMiddlePointY, 5);

  var middleBottomPointX = lerp(points.data[2][0], points.data[3][0], easedT);
  var middleBottomPointY = lerp(points.data[2][1], points.data[3][1], easedT);
  point(middleBottomPointX, middleBottomPointY, 5);

  var bottomRightPointX = lerp(points.data[3][0], points.data[1][0], easedT);
  var bottomRightPointY = lerp(points.data[3][1], points.data[1][1], easedT);
  point(bottomRightPointX, bottomRightPointY, 5);
  ...
  var midpoint1X = lerp(topMiddlePointX, middleBottomPointX, easedT);
  var midpoint1Y = lerp(topMiddlePointY, middleBottomPointY, easedT);
  point(midpoint1X, midpoint1Y, 5);

  var midpoint2X = lerp(middleBottomPointX, bottomRightPointX, easedT);
  var midpoint2Y = lerp(middleBottomPointY, bottomRightPointY, easedT);
  point(midpoint2X, midpoint2Y, 5);
  line(midpoint1X, midpoint1Y, midpoint2X, midpoint2Y);
  var finalMidPointX = lerp(midpoint1X, midpoint2X, easedT);
  var finalMidPointY = lerp(midpoint1Y, midpoint2Y, easedT);
  point(finalMidPointX, finalMidPointY, 10);
  /*
  Trail
  ctx2.beginPath();
...
function drawMidPoints() {
  ...
  for (var i = 0; i < points.computed.length; i++) {
    point(points.computed[i][0], points.computed[i][1], 5);
    if (i != points.computed.length - 1)
      line(
        points.computed[i + 1][0],
        points.computed[i + 1][1],
        points.computed[i][0],
        points.computed[i][1]
      );
  }
...
function clearComputed() {
  ...
  for (var i = 0; i < points.data.length; i++) {
    for (var j = 0; j < points.data.length; j++) {
      if (iteration == 0) points.computed.push(points.data[i][j]);
    }
  }
}
